...
def index_retrieve(index, query_embeddings, topk,device, batch=None):
    ...
    if batch is None:
        ...
        while query_offset_base < len(query_embeddings):
            ...
            score,batch_nn = index.search(batch_query_embeddings, topk)
            nearest_neighbors.extend(batch_nn.tolist())
            score_neatest_neighbors.extend(score.tolist())
    ...
    return torch.tensor(nearest_neighbors).to(device),torch.tensor(score_neatest_neighbors).to(device)
